...
public class VineAudioTrigger : MonoBehaviour
{
    ...
    void Update()
    {
        // Stop the vine audio if the Aster flower is collected (destroyed)
        if (asterFlower == null && vineAudioSource.isPlaying)
        {
            vineAudioSource.Stop();
            Debug.Log("Aster flower collected. Stopping vine audio.");
        }
    }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.CompareTag("Player") && asterFlower != null)
        {
            ...
            if (!vineAudioSource.isPlaying)
            {
                ...
                Debug.Log("Player entered vine trigger. Playing vine audio.");
            }
        }
    }
}
